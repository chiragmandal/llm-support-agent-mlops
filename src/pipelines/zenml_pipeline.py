"""
ZenML pipeline: ingest -> eval -> register.

Key changes vs original:
- ingest_step and eval_step now import and call functions directly instead
  of spawning subprocesses via subprocess.check_call(). The subprocess
  approach forks a whole Python interpreter, bypasses ZenML's step-level
  error handling and logging, and makes stack traces unreadable in the UI.
  Direct function calls are faster, debuggable, and idiomatic ZenML.
- All steps use structured logging via the standard library.
"""
import json
import logging
import os
...
from src.apps.ingestor.ingest import main as run_ingest
from src.apps.evaluator.eval import run_eval
...
@step
def ingest_step() -> str:
    """Ingest KB documents into Qdrant."""
    logger.info("Starting ingest step")
    run_ingest()
    logger.info("Ingest step complete")
    return "ingest_ok"


@step
def eval_step(ingest_status: str) -> dict:
    """Run evaluation suite and return the report dict."""
    logger.info("Starting eval step (ingest_status=%s)", ingest_status)
    report = run_eval()
    logger.info("Eval step complete: %s", report.get("metrics", {}))
    return report


@step
def register_step(report: dict) -> bool:
    """Log metrics to MLflow and decide whether the quality gate passes."""
    ...
    logger.info("gate_pass=%s metrics=%s", gate_pass, metrics)
    return gate_pass
...
def llm_support_pipeline():
    ...
    report = eval_step(ingest_status)
